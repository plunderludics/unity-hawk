...
- The `Emulator` component provides an interface to basic Bizhawk API methods.
  - For controlling the emulator: `Pause()`, `Unpause()`, `FrameAdvance()`, `LoadState(Savestate s)`, `SaveState(string path)`, `LoadRom(Rom r)`. Use the `BasicApiTool` component to use these methods directly from the Editor.
